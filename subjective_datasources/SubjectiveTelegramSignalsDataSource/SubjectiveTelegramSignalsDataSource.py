...
class SubjectiveTelegramSignalsDataSource(SubjectiveDataSource):
    connection_type = "Telegram"
    connection_fields = ["api_id", "api_hash", "channels"]
    ...
    def fetch(self):
        ...
        update_channels = bool(self._get_param("update_channels", False))
        crawler = TelegramCrawler(update_channels=update_channels)
        crawler.crawl_for_signals()
