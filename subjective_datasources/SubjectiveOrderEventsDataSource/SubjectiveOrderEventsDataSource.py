...
class SubjectiveOrderEventsDataSource(SubjectiveDataSource):
    connection_type = "Orders"
    connection_fields = ["exchange", "account_id"]
    ...
    def fetch(self):
        ...
        orders = MongoConnector.get_instance().query_collection(collection_name="orders", query={})
        self._emit_result({"orders": list(orders)})
