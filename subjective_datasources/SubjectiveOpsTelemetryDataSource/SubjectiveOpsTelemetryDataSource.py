...
class SubjectiveOpsTelemetryDataSource(SubjectiveDataSource):
    connection_type = "OpsTelemetry"
    connection_fields = ["service_name", "host", "interval"]
    ...
    def fetch(self):
        ...
        processes = self._get_param("processes", [])
        self._emit_result({"processes": processes, "status": "ops_telemetry_placeholder"})
